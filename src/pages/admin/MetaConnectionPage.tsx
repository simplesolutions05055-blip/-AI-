...
export default function MetaConnectionPage() {
  ...
  const loadConnectionData = async () => {
    try {
      ...
      setData(connectionData as ConnectionData | null);
      
      if (connectionData) {
        console.log('✅ Connection data loaded:');
        console.log(`- User: ${connectionData.meta_user_name}`);
        console.log(`- Pages: ${connectionData.meta_facebook_pages?.length || 0}`);
        console.log(`- Instagram: ${connectionData.meta_instagram_accounts?.length || 0}`);
      } else {
