...
export default function BrandAutofillPanel({ initialQuery, onApply }: {
  ...
  async function run() {
    ...
    setLoading(true); setElapsed(0); setError(null); setResult(null);
    try {
      ...
      setResult(payload);
      setSelected(new Set(payload.fields.filter((field) => field.state === 'trusted').map((field) => field.key).concat(payload.colors.length ? ['color_palette'] : [])));
  ...
  return <div className="mb-6 rounded-2xl border border-brand/25 bg-brand/5 p-4">
    ...
    <p className="mt-1 text-sm text-[var(--muted)]">הזינו אתר או שם. שום דבר לא נשמר לפני שתאשרו.</p>
    ...
    {result && <div className="mt-5">
      ...
      <div className="space-y-2">
        {result.fields.map((field) => <label key={field.key} className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 ${field.state === 'review' ? 'border-amber-300 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
