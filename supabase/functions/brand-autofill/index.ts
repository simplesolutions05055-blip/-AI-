    ...
    body: JSON.stringify({
      ...
      input: [
        ...
        { role: 'user', content: JSON.stringify({
          ...
          schema: { official_name: 'string|null', short_name: 'string|null', website: 'string|null', address: 'string|null', postal_code: 'string|null', phone: 'string|null', fax: 'string|null', email: 'string|null', legal_id: 'string|null', contact_person_name: 'string|null', contact_person_title: 'string|null', logo_url: 'Direct https link to an actual image FILE (png/jpg/svg — not a webpage) of the official emblem/logo, or null. Strongly prefer a Wikimedia Commons upload.wikimedia.org file link (search Wikipedia/Commons for the organization\'s emblem/coat of arms first) since those are stable and directly fetchable; only use a link from the official site itself if no Commons file exists. Never return a link to an HTML page.', sources: '{field: url}' },
        }) },
      ],
    }),
  });
