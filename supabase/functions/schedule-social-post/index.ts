...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (platforms.length === 0 || platforms.some((p) => p !== 'facebook' && p !== 'instagram')) {
      return json(req, { error: 'invalid_platform' }, 400);
    }
    ...
    if (platforms.includes('instagram') && media.length === 0 && !body.image_url) {
      return json(req, { error: 'instagram_requires_media' }, 400);
    }
    ...
    if (!resolved) {
      return json(req, { error: 'meta_not_connected' }, 400);
    }
    ...
    if (body.connection_id && body.connection_id !== connection.connection_id) {
      ...
      if (callerConnection?.connection_id !== body.connection_id) {
        return json(req, { error: 'connection_mismatch' }, 400);
      }
    ...
    for (const platform of platforms) {
      ...
      if (explicit) {
        ...
        if (!match) return json(req, { error: 'invalid_target', platform }, 400);
        ...
        if (!fallback) return json(req, { error: 'meta_target_required', platform }, 400);
    ...
    return json(req, { ok: true, schedule: data?.[0] ?? null, schedules: data ?? [] });
  } catch (e) {
